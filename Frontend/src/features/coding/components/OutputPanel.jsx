...
const OutputPanel = ({ result, isRunning = false }) => {
  ...
  return (
    <section
      className={`coding-output ${collapsed ? "is-collapsed" : ""} ${successful ? "is-success" : result ? "is-error" : ""}`}
      aria-live="polite"
    ...
    >
      <div className="coding-output__heading">
        ...
        <div>
          <span>
            {successful ? "✓ " : result && !isRunning ? "✗ " : ""}
            {STATUS_LABELS[status] || "Execution completed"}
          </span>
          <button
            type="button"
