...
const Dashboard = () => {
  ...
  useEffect(() => {
    getRecommendedJobs()
      .then(setRecommended)
      .catch(() => setRecommended({ jobs: [], missingProfile: false }));
  }, []);

  const prepareRecommended = async (job) => {
    try {
      const data = await prepareJob(job._id);
  ...
  const highMatches = reports.filter(
    (report) => report.matchScore >= 80,
  ).length;
  const averageMatch = totalPlans
    ? Math.round(
        reports.reduce((total, report) => total + (report.matchScore || 0), 0) /
          totalPlans,
      )
    : 0;
  ...
  if (loading) {
    return (
      <div className="dashboard-page">
        <AppHeader />
        <main
          className="dashboard-main dashboard-loading"
          aria-label="Loading dashboard"
        >
          <span className="dashboard-loading__hero" />
    ...
    <div className="dashboard-page">
      ...
      <main className="dashboard-main">
        <section className="dashboard-hero">
          <div>
            ...
            <p className="dashboard-hero__copy">
              Build focused preparation plans, review your strongest
              opportunities, and keep moving toward interview readiness.
            </p>
          </div>
          <button
            type="button"
        ...
        <section className="dashboard-tools" aria-label="Practice shortcuts">
            ...
            <div>
              ...
              <span>
                {streak?.currentStreak
                  ...
                  : "Complete meaningful practice to begin."}
              </span>
            </div>
            <div className="dashboard-week">
              {(streak?.week?.length
                ...
                : ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                    (label) => ({ label, date: label, active: false }),
                  )
              ).map((day) => (
                <span key={day.date} className={day.active ? "active" : ""}>
                  {day.label}
                  <b
                    aria-label={
                      day.active
          ...
          <article className="dashboard-quick">
            <p>Quick practice</p>
            <div>
              <button onClick={() => navigate("/focused-practice")}>
                Technical questions
              </button>
              <button onClick={() => navigate("/coding-practice")}>
                Coding practice
              </button>
              <button onClick={() => navigate("/mcq")}>MCQ practice</button>
              <button onClick={() => navigate("/question-bank")}>
                Saved questions
              </button>
              <button onClick={() => navigate("/project-questions")}>
                Project questions
              </button>
              <button onClick={() => navigate("/introductions")}>
        ...
        <section className="dashboard-recent" aria-labelledby="recent-title">
          <div className="dashboard-section-heading dashboard-section-heading--row">
            ...
            {recentPlans.length > 0 && (
              <button
                ...
                onClick={() => navigate("/interviews/create")}
              >
                Create another <span aria-hidden="true">&rarr;</span>
              </button>
            )}
          </div>
          ...
          {recentPlans.length > 0 ? (
            <div className="dashboard-plans">
              {recentPlans.map((report) => (
                ...
                >
                  ...
                  <span className="dashboard-plan__link">
                    Open preparation plan <span aria-hidden="true">&rarr;</span>
                  </span>
                </button>
              ))}
            </div>
            ...
            <div className="dashboard-empty">
              ...
              <p>
                Add a job description and resume to receive a focused interview
                strategy.
              </p>
              <button
                type="button"
        ...
        >
          ...
          {recommended.jobs.length > 0 ? (
              ...
              {recommended.jobs.map((job) => (
                <article key={job._id}>
                  ...
                  <div className="dashboard-job-actions">
                    <button
                      className="dashboard-job-actions__view"
                      type="button"
                    ...
                    <button
                      className="dashboard-job-actions__prepare"
                      type="button"
                      onClick={() => prepareRecommended(job)}
                    >
                      Prepare interview <span aria-hidden="true">→</span>
                    </button>
                  </div>
                </article>
              ))}
            ...
            <div className="dashboard-empty">
              <h3>
                {recommended.missingProfile
                  ? "Complete your candidate profile"
                  : "No recommendations yet"}
              </h3>
              <p>
                {recommended.missingProfile
                  ? "Create an interview plan with your resume or self-description to unlock relevant job recommendations."
                  : "We will show current roles here as they become available."}
              </p>
              <button
                type="button"
                onClick={() =>
                  navigate(
                    recommended.missingProfile ? "/interviews/create" : "/jobs",
                  )
                }
              >
                {recommended.missingProfile
