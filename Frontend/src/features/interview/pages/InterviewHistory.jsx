...
const InterviewHistory = () => {
  ...
  const { loading, error, reports, removeReport, getResumePdf } =
    useInterview();
  ...
  const deleteReport = async (report) => {
    const confirmed = window.confirm(
      `Delete "${report.title || "Untitled position"}"? This cannot be undone.`,
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setActionError(
        requestError.response?.data?.message ||
          "Unable to delete this interview plan.",
      );
    } finally {
      setDeletingId("");
    }
  ...
  const startMock = async (reportId) => {
    ...
    try {
      ...
      setActionError(
        requestError.response?.data?.message ||
          "Unable to start a mock interview.",
      );
    } finally {
      setStartingId("");
    }
      ...
      <main className="history-main">
        <header className="history-heading">
          <div>
            ...
            <span>
              Review and manage every strategy generated for your target roles.
            </span>
          </div>
          <button type="button" onClick={() => navigate("/interviews/create")}>
            Create interview plan <span aria-hidden="true">&rarr;</span>
          </button>
        </header>
            ...
            {filteredReports.map((report) => (
              <li key={report._id} className="history-card">
                ...
                <div className="history-card__actions">
                  <button
                    type="button"
                    className="history-card__mock"
                    onClick={() => startMock(report._id)}
                    disabled={startingId === report._id}
                  >
                    {startingId === report._id
                      ? "Starting..."
          ...
          <div className="history-state">
            ...
            <p>
              {search
                ...
                : "Create a plan for the next role you are targeting."}
            </p>
          </div>
        )}
      </main>
