...
const QuestionBank = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await getQuestionBank({ search, category, sort });
  ...
  const remove = async (id) => {
    try {
      ...
      setQuestions((items) => items.filter((item) => item._id !== id));
    } catch (requestError) {
      setError(
        requestError.response?.data?.message || "Unable to remove question.",
      );
    }
  };
  return (
      ...
      <main className="prep-main">
        <header className="prep-heading">
          ...
          <span>
            ...
            . Revisit difficult questions and turn them into focused timed
            practice.
          </span>
        </header>
        <section className="prep-card prep-controls">
          ...
          <div className="prep-field">
            <label htmlFor="bank-sort">Sort</label>
            <select
              id="bank-sort"
              value={sort}
        ...
        {loading ? (
          <div className="prep-state">Opening your question library...</div>
        ) : questions.length === 0 ? (
          <div className="prep-card prep-state">
            <h2>No saved questions yet</h2>
            <p>
              Save questions while reviewing an interview plan, then practice
              them here.
            </p>
            <button
                ...
                <div>
                  ...
                  {q.personalAnswer && (
                    <details className="bank-answer">
                      <summary>
                        Your saved answer{" "}
                        {q.evaluationSnapshot?.score != null &&
                          `· ${q.evaluationSnapshot.score}%`}
                      </summary>
                      <h3>Your answer</h3>
                      <p>{q.personalAnswer}</p>
                      {q.evaluationSnapshot?.feedback && (
                        <>
                          <h3>Feedback</h3>
                          <p>{q.evaluationSnapshot.feedback}</p>
                        </>
                      )}
                    </details>
                  )}
