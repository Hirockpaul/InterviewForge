...
export const startMockInterview = async (interviewPlanId, focus) => {
    const response = await api.post('/api/mock-interviews', { interviewPlanId, focus })
...
export const submitMockAnswer = async (mockInterviewId, answer) => {
    const response = await api.post(`/api/mock-interviews/${mockInterviewId}/answer`, { answer })
    return response.data
}
