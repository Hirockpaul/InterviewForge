...
const MockInterview = () => {
      ...
      try {
        ...
        setError(
          requestError.response?.data?.message ||
            "Unable to load this mock interview.",
        );
      } finally {
        setLoading(false);
      }
  ...
  if (loading)
    return <main className="mock-state">Preparing your mock interview...</main>;
  if (error && !session)
    return (
      <main className="mock-state">
        <h1>Mock interview unavailable</h1>
        <p>{error}</p>
      ...
      <main className="mock-main">
        <header className="mock-heading">
          ...
          <span>
            {session.status === "completed"
              ? "Complete"
              : `Question ${session.questions.length} of 5`}
          </span>
        </header>
        ...
        {session.status === "completed" ? (
          <section className="mock-results">
            ...
            <span>
              Review your latest feedback, then return to your preparation plan
              to keep improving.
            </span>
          ...
          <section className="mock-question">
            ...
            <div className="prep-actions">
              <SaveQuestionButton
                question={{
                  ...
                  source: "mock",
                  category:
                    currentQuestion.questionType === "behavioral"
                      ? "behavioral"
              ...
              <button
                ...
                onClick={() =>
                  navigate("/focused-practice", {
                    state: { interviewPlanId: session.interviewPlan?._id },
                  })
                }
              >
                Technical questions
            ...
            <form onSubmit={submitAnswer}>
              ...
              <textarea
                ...
                placeholder="Explain your thinking clearly and include a practical example where possible..."
              />
              <button
                type="submit"
        ...
        {error && (
          <p className="mock-error" role="alert">
            {error}
          </p>
        )}
        ...
        {latestEvaluation && (
          <section className="mock-feedback">
            <div className="mock-feedback__score">
            ...
            <div className="mock-feedback__metrics">
              <span>
                Technical accuracy{" "}
                <strong>{latestEvaluation.technicalAccuracy}%</strong>
              </span>
              <span>
                Communication <strong>{latestEvaluation.communication}%</strong>
              </span>
              <span>
                Clarity{" "}
                <strong>
                  {latestEvaluation.clarity ?? "--"}
                  {latestEvaluation.clarity != null ? "%" : ""}
                </strong>
              </span>
              <span>
                Depth <strong>{latestEvaluation.depth}%</strong>
              </span>
              <span>
                Relevance{" "}
                <strong>
                  {latestEvaluation.relevance ?? "--"}
                  {latestEvaluation.relevance != null ? "%" : ""}
                </strong>
              </span>
            </div>
