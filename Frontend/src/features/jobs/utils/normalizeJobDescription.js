...
function decodeEntities(value) {
  ...
  for (
    let pass = 0;
    pass < 2 && /&(?:lt|gt|amp|quot|#\d+|#x[a-f0-9]+);/i.test(decoded);
    pass += 1
  ) {
...
export function normalizeJobDescription(
  ...
  if (firstHeading && cleanText(firstHeading.textContent) === cleanText(title))
    firstHeading.remove();

  const companyName = cleanText(company);
  const companyHeading = [...body.querySelectorAll(headingSelector)].find(
    (heading) => {
      const text = cleanText(heading.textContent);
      return (
        /^about (the )?company$/.test(text) ||
        (companyName && text === `about ${companyName}`)
      );
    },
  );
  const companyContainer = document.createElement("div");
  if (companyHeading) {
    const isGenericCompanyHeading = /^about (the )?company$/.test(
      cleanText(companyHeading.textContent),
    );
  ...
  return {
    roleHtml: body.innerHTML.trim(),
    companyHtml: companyContainer.innerHTML.trim(),
  };
}
