...
const JobCard = ({ job, onPrepare, onSave }) => {
    ...
    return <article className='job-card'>
        ...
        {job.skills?.length > 0 && <div className='job-skills'>{job.skills.slice(0, 6).map((skill) => <span key={skill}>{skill}</span>)}</div>}
        <div className='job-card__footer'><div className='job-card__provenance'><span className='job-card__posted'>{relativeDate(job.postedAt)}</span><span>Source: {job.source === 'jobdatalake' ? 'JobDataLake' : job.source === 'jsearch' ? 'JSearch' : 'Adzuna'}</span></div><div><Link className='job-button job-button--quiet' to={`/jobs/${job._id}`}>View job</Link><button className='job-button' type='button' onClick={() => onPrepare(job)}>Prepare interview <span aria-hidden='true'>→</span></button></div></div>
    </article>
}
