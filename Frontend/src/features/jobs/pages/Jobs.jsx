...
const Jobs = () => {
    ...
    return <div className='jobs-page'><AppHeader /><main className='jobs-main'>
        ...
        <div className='jobs-layout'>
            ...
            <section className='jobs-results' aria-live='polite'><div className='jobs-results__heading'><div><h2>{data.pagination.total ?? 0} jobs found</h2>{data.metadata.lastUpdated && <p>Updated {new Date(data.metadata.lastUpdated).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>}</div><div className='jobs-results__controls'><button className='jobs-filter-toggle' type='button' aria-expanded={filtersOpen} onClick={() => setFiltersOpen((open) => !open)}>Filters</button><label htmlFor='job-sort'>Sort <select id='job-sort' value={draft.sort} onChange={(e) => { set('sort', e.target.value); load({ ...filters, sort: e.target.value, page: 1 }) }}><option value='newest'>Newest</option><option value='best-match'>Best match</option><option value='salary-desc'>Salary high → low</option><option value='salary-asc'>Salary low → high</option><option value='relevance'>Relevance</option></select></label></div></div>
                ...
                {!error && !loading && data.jobs.length === 0 && <div className='jobs-state'><h3>No jobs found</h3><p>Try changing your location, filters, or search terms.</p><button className='job-button' type='button' onClick={() => { setDraft(initial); load(initial) }}>Clear filters</button></div>}
                ...
                {!loading && data.jobs.length > 0 && <nav className='jobs-pagination' aria-label='Job results pages'><button disabled={filters.page <= 1} onClick={() => load({ ...filters, page: filters.page - 1 })}>← Previous</button><span>Page {filters.page} of {Math.max(1, Math.ceil((data.pagination.total || 0) / (data.pagination.limit || filters.limit)))}</span><button disabled={!data.pagination.hasNext} onClick={() => load({ ...filters, page: filters.page + 1 })}>Next →</button></nav>}
            </section>
        </div>
    </main></div>
}
