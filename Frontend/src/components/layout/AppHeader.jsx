...
import interviewForgeLogo from "../../assets/images/interview-forge-logo.svg";
...
const groups = [
  ...
  },
  {
    label: "Career",
    items: [
      { label: "Search jobs", to: "/jobs", exact: true, icon: FiSearch },
      { label: "Saved jobs", to: "/jobs/saved", icon: FiBookmark },
      { label: "Applications", to: "/jobs/applications", icon: FiBriefcase },
    ],
  },
  {
    label: "Preparation",
  ...
  {
    label: "Insights",
    items: [{ label: "Progress", to: "/progress", icon: FiBarChart2 }],
  },
];
...
const AppHeader = () => {
  ...
  const [collapsed, setCollapsed] = useState(
    () => localStorage.getItem("if-sidebar-collapsed") === "true",
  );
  ...
  useEffect(() => {
    document.body.classList.toggle("if-sidebar-collapsed", collapsed);
    localStorage.setItem("if-sidebar-collapsed", collapsed);
    return () => document.body.classList.remove("if-sidebar-collapsed");
  }, [collapsed]);
  useEffect(() => {
  ...
  const active = (item) =>
    item.exact
      ...
      : pathname === item.to ||
        ...
        item.aliases?.some((path) => pathname.startsWith(path));
  const logout = async () => {
    setLoggingOut(true);
    try {
      await handleLogout();
  ...
  return (
    <>
      ...
      <aside
        className={`app-header ${collapsed ? "is-collapsed" : ""} ${mobileOpen ? "is-mobile-open" : ""}`}
      ...
      >
        <div className="side-nav__brand-row">
          <Link
            className="app-brand"
            to="/dashboard"
            title={collapsed ? "InterviewForge" : undefined}
          >
            <span className="app-brand__mark">
              <img src={interviewForgeLogo} alt="" />
            </span>
            <span className="app-brand__name">InterviewForge</span>
          </Link>
          <button
            className="side-nav__mobile-close"
            type="button"
            aria-label="Close navigation"
            onClick={() => setMobileOpen(false)}
          >
        ...
        <div className="side-nav__footer">
          ...
          <div className="side-nav__account">
            ...
            <span>
              ...
              <small>{user?.email || "Your profile"}</small>
            </span>
            <button
              type="button"
              onClick={logout}
