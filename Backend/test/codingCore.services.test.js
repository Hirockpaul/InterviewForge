...
const validProblem = {
    ...
    shortDescription: 'Find a position where the values on both sides have equal sums.',
    description: 'Given an integer array, return the first index that divides it into non-empty left and right portions with equal sums. Return -1 when no such position exists.',
    difficulty: 'easy', topic: 'arrays',
    constraints: [ '2 <= nums.length <= 100000' ],
    examples: [
        { input: '[1, 2, 3, 3]', output: '3', explanation: 'Both portions sum to 6.' },
    ...
    hints: [ 'Track the total before choosing a split.', 'Update a running prefix sum.' ],
    explanation: 'Compare a running left sum with the remaining total at every valid boundary.',
...
test('coding topics expose the complete centralized catalog', () => {
    ...
    assert.deepEqual(CODING_TOPICS[0], {
        id: 'arrays', displayName: 'Arrays', slug: 'arrays',
        shortDescription: 'Practice array traversal, manipulation, and problem solving.'
    })
})

test('problem query validates supported topic, difficulty, and search', () => {
    assert.equal(problemQuerySchema.parse({ topic: 'arrays', difficulty: 'medium', search: 'sum' }).difficulty, 'medium')
    assert.equal(problemQuerySchema.safeParse({ topic: 'unknown' }).success, false)
    assert.equal(problemQuerySchema.safeParse({ difficulty: 'expert' }).success, false)
})

test('AI generation request requires a supported topic and exact difficulty', () => {
    assert.deepEqual(generationSchema.parse({ topic: 'graphs', difficulty: 'hard' }), { topic: 'graphs', difficulty: 'hard' })
    assert.equal(generationSchema.safeParse({ topic: 'graphs', difficulty: 'expert' }).success, false)
})
...
test('problem listing sends topic, difficulty, and escaped search to MongoDB', async () => {
    ...
    let receivedQuery
    const chain = { select() { return this }, sort() { return this }, skip() { return this }, limit() { return this }, lean: async () => [ { title: 'Sum Pair' } ] }
    codingProblemModel.find = (query) => { receivedQuery = query; return chain }
    codingProblemModel.countDocuments = async () => 1
    try {
        const result = await codingService.listProblems({ topic: 'arrays', difficulty: 'easy', search: 'sum.', page: 1, limit: 24 })
...
test('generation reuses a full MongoDB pool without calling Gemini', async () => {
    ...
    let aiCalled = false
    const existing = Array.from({ length: 10 }, (_, index) => ({ title: `Problem ${index}`, normalizedTitle: `problem ${index}` }))
    const chain = { select() { return this }, lean: async () => existing }
    codingProblemModel.find = () => chain
    codingAiService.generateCodingProblems = async () => { aiCalled = true; return [] }
    try {
        const result = await codingService.generateMissing({ topic: 'arrays', difficulty: 'easy' })
...
test('coding routes apply authentication globally and AI limiting to generation', () => {
    ...
    const generationRoute = router.stack.find((layer) => layer.route?.path === '/problems/generate')
