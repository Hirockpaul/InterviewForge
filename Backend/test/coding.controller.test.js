...
test('run endpoint executes valid JavaScript and returns output 5', async () => {
    ...
    try {
        ...
        await runCodeController({ body: { language: 'javascript', versionIndex: '4', code: 'console.log(2 + 3);', stdin: '' } }, res)

        assert.equal(res.statusCode, 200)
        assert.equal(res.body.output, '5')
        assert.deepEqual(received, { language: 'nodejs', versionIndex: '4', code: 'console.log(2 + 3);', stdin: '' })
    } finally {
...
test('run endpoint returns normalized JDoodle errors', async () => {
    ...
    try {
        const res = responseRecorder()
        await runCodeController({ body: { language: 'javascript', code: 'console.log(2 + 3);' } }, res)
