...
function profileUser(overrides = {}) {
  return {
    _id: {
      toString: () => "user-one",
      getTimestamp: () => new Date("2025-01-01"),
    },
    username: "Hirock",
    email: "hirock@example.com",
...
test("authenticated user can update display name and avatar without changing account data", async () => {
  ...
  try {
    ...
    await updateProfileController(
      {
        user: { id: "user-one" },
        body: { displayName: "Hirock Gogoi", avatarStyle: "adventurer" },
      },
      res,
    );
...
test("invalid avatar style is rejected before accessing MongoDB", async () => {
  ...
  try {
    ...
    await updateProfileController(
      {
        user: { id: "user-one" },
        body: { displayName: "Hirock", avatarStyle: "untrusted-style" },
      },
      res,
    );
...
test("unauthenticated profile request is rejected by existing auth middleware", async () => {
  const res = responseRecorder();
  await authUser({ cookies: {} }, res, () =>
    assert.fail("middleware should not continue"),
  );
...
test("authentication rejects an access token after its session version is revoked", async () => {
  ...
  userModel.findById = () => ({
    ...
    lean: async () => ({ sessionVersion: 2 }),
  });
  const token = jwt.sign(
    { id: "user-one", tokenType: "access", sessionVersion: 1 },
    process.env.JWT_SECRET,
  );
  try {
    const res = responseRecorder();
    await authUser({ cookies: { token } }, res, () =>
      assert.fail("revoked session should not continue"),
    );
    assert.equal(res.statusCode, 401);
  } finally {
...
test("missing avatar fields receive and persist a stable fallback on profile load", async () => {
  ...
  try {
    ...
    await getMeController(
      { user: { id: "user-one" }, cookies: { refreshToken: "present" } },
      res,
    );
...
test("serialized existing profile includes a stable member date and avatar", () => {
  ...
  assert.equal(
    serialized.memberSince.toISOString(),
