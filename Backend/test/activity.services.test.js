...
test("returns an empty streak for an existing user with no activity", () => {
  assert.deepEqual(
    calculateStreak([], "UTC", new Date("2026-08-22T12:00:00Z")),
    {
      currentStreak: 0,
      longestStreak: 0,
      totalActiveDays: 0,
...
test("counts multiple activities on one day once", () => {
  const result = calculateStreak(
    [activity("2026-08-22T01:00:00Z"), activity("2026-08-22T22:00:00Z")],
    "UTC",
...
test("handles consecutive days, missed days, and longest streak", () => {
  const values = [
    ...
    "2026-08-20",
    "2026-08-21",
  ].map((day) => activity(`${day}T12:00:00Z`));
  const result = calculateStreak(
    values,
    "UTC",
...
test("resets the current streak after a missed day without losing the longest streak", () => {
  const values = ["2026-08-14", "2026-08-15", "2026-08-16"].map((day) =>
    activity(`${day}T12:00:00Z`),
  );
  const result = calculateStreak(
    values,
    "UTC",
...
test("uses the candidate timezone when assigning calendar days", () => {
  const values = [
    activity("2026-08-21T23:30:00Z"),
    activity("2026-08-22T22:30:00Z"),
  ];
  const result = calculateStreak(
    values,
    "Asia/Kolkata",
