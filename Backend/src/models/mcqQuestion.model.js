...
const mcqQuestionSchema = new mongoose.Schema(
  {
    ...
    options: {
      ...
      validate: [
        (items) =>
          items.length === 4 &&
          new Set(items.map((item) => item.toLowerCase().trim())).size === 4,
        "Exactly four unique options are required.",
      ],
    },
    ...
    license: String,
    visibility: { type: String, enum: ["global", "user"], default: "global" },
    owner: {
...
mcqQuestionSchema.index(
  { normalizedQuestion: 1, visibility: 1, owner: 1 },
  { unique: true },
);
mcqQuestionSchema.pre("validate", function validateVisibility() {
  if (this.visibility === "user" && !this.owner) {
    this.invalidate("owner", "Private MCQs require an owner.");
  }
