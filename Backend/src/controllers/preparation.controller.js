...
const questionSchema = z.object({
    ...
    evaluationSnapshot: z.object({
        score: z.number().min(0).max(100).optional(), strengths: z.array(z.string()).max(6).optional().default([]),
        weaknesses: z.array(z.string()).max(6).optional().default([]), feedback: z.string().max(4000).optional().default(''),
        recommendedImprovement: z.string().max(2000).optional().default('')
    }).optional()
})
const introductionGenerateSchema = z.object({
    interviewPlanId: z.string(), targetRole: z.string().trim().min(2).max(120), duration: z.union([ z.literal(30), z.literal(60), z.literal(120) ]),
    tone: z.enum([ 'professional', 'confident', 'natural' ])
})

const parse = (schema, body, res) => {
    const result = schema.safeParse(body)
    if (!result.success) res.status(400).json({ message: result.error.issues[0].message })
...
async function listQuestions(req, res) {
    ...
    if (req.query.search) query.questionText = { $regex: String(req.query.search).slice(0, 100).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
    const sort = req.query.sort === 'oldest' ? { createdAt: 1 } : req.query.sort === 'question' ? { questionText: 1 } : { createdAt: -1 }
    return res.json({ questions: await savedQuestionModel.find(query).sort(sort) })
}

async function saveQuestion(req, res) {
    const data = parse(questionSchema, req.body, res); if (!data) return
    try {
        const question = await savedQuestionModel.create({ user: req.user.id, ...data, normalizedText: normalize(data.questionText) })
        return res.status(201).json({ question, message: 'Question saved.' })
    } catch (error) {
        if (error.code === 11000) {
            const question = await savedQuestionModel.findOneAndUpdate(
                { user: req.user.id, normalizedText: normalize(data.questionText) },
...
async function removeQuestion(req, res) {
    if (!validId(req.params.id)) return res.status(400).json({ message: 'Invalid question ID.' })
    const removed = await savedQuestionModel.findOneAndDelete({ _id: req.params.id, user: req.user.id })
    if (!removed) return res.status(404).json({ message: 'Saved question not found.' })
...
async function createIntroduction(req, res) {
    const data = parse(introductionGenerateSchema, req.body, res); if (!data) return
    if (!validId(data.interviewPlanId)) return res.status(400).json({ message: 'Invalid interview plan ID.' })
    const plan = await interviewReportModel.findOne({ _id: data.interviewPlanId, user: req.user.id })
    if (!plan) return res.status(404).json({ message: 'Interview plan not found.' })
    if (!(plan.resume || plan.selfDescription).trim()) return res.status(422).json({ message: 'Add resume or profile information before generating an introduction.' })
...
async function updateIntroduction(req, res) {
    if (!validId(req.params.id)) return res.status(400).json({ message: 'Invalid introduction ID.' })
    const data = parse(z.object({ content: z.string().trim().min(20).max(12000) }), req.body, res); if (!data) return
    ...
    if (!introduction) return res.status(404).json({ message: 'Introduction not found.' })
...
async function listProjects(req, res) {
    if (!validId(req.query.interviewPlanId)) return res.status(400).json({ message: 'A valid interview plan ID is required.' })
...
async function generateQuestions(req, res) {
    const data = parse(z.object({ interviewPlanId: z.string(), projectName: z.string().trim().min(1).max(200), projectContext: z.string().trim().min(10).max(5000) }), req.body, res); if (!data) return
    if (!validId(data.interviewPlanId)) return res.status(400).json({ message: 'Invalid interview plan ID.' })
    const plan = await interviewReportModel.findOne({ _id: data.interviewPlanId, user: req.user.id })
    if (!plan) return res.status(404).json({ message: 'Interview plan not found.' })
    ...
    if (!verified) return res.status(422).json({ message: 'That project was not found in your candidate information.' })
...
async function getStreak(req, res) {
    ...
    try { new Intl.DateTimeFormat('en', { timeZone }) } catch { return res.status(400).json({ message: 'Invalid timezone.' }) }
...
module.exports = { listQuestions, saveQuestion, removeQuestion,
    listIntroductions, createIntroduction, updateIntroduction, deleteIntroduction, practiceIntroduction, listProjects, generateQuestions, getStreak }
