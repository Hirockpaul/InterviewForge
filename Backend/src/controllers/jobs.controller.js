...
const querySchema = z.object({
    ...
    posted: z.enum(['today', '3d', '7d', '30d']).optional(),
...
function parseQuery(req, res) {
    ...
    return {
        ...
        location,
        postedDays: days,
        postedAfter: days ? new Date(Date.now() - days * 86400000) : undefined
    }
}

async function list(req, res) {
    const params = parseQuery(req, res)
    if (!params) {
        return
    }
    const result = await jobsService.searchJobs(params)
    const savedJobs = await SavedJob.find({ user: req.user.id }).select('job').lean()
    const savedIds = new Set(savedJobs.map(item => String(item.job)))
    const scores = await JobAnalysis.find({
        user: req.user.id,
        job: { $in: result.jobs.map(job => job._id) }
    }).sort({ createdAt: -1 }).select('job matchScore').lean()
    const scoreByJob = new Map(scores.map(analysis => [String(analysis.job), analysis.matchScore]))

    if (params.sort === 'best-match') {
        result.jobs.sort((first, second) => (
            (scoreByJob.get(String(second._id)) ?? -1)
            - (scoreByJob.get(String(first._id)) ?? -1)
        ))
    }
    ...
    return res.json({
        jobs: result.jobs.map(job => ({
            ...
            matchScore: scoreByJob.get(String(job._id))
        })),
        pagination: {
            page: params.page,
            limit: params.limit,
        ...
        metadata: {
            sources: result.sources.length ? result.sources : [...new Set(result.jobs.map(job => job.source))],
            lastUpdated: result.jobs[0]?.lastSeenAt || null,
...
async function details(req, res) {
    ...
    const analysis = await JobAnalysis.findOne({ user: req.user.id, job: job._id })
        .sort({ createdAt: -1 })
        .lean()
...
async function save(req, res) {
    if (!mongoose.isValidObjectId(req.params.id) || !await Job.exists({ _id: req.params.id })) {
        return res.status(404).json({ message: 'Job not found.' })
    }
...
async function saved(req, res) {
    const records = await SavedJob.find({ user: req.user.id })
        .sort({ savedAt: -1 })
        .populate('job')
        .lean()
    const activeRecords = records.filter(record => record.job)
    const analyses = await JobAnalysis.find({
        user: req.user.id,
        job: { $in: activeRecords.map(record => record.job._id) }
    }).sort({ createdAt: -1 }).select('job matchScore').lean()
    const scores = new Map()
    for (const analysis of analyses) {
        if (!scores.has(String(analysis.job))) {
            scores.set(String(analysis.job), analysis.matchScore)
        }
    }
    return res.json({
        jobs: activeRecords.map(record => ({
            ...record.job,
            saved: true,
            savedAt: record.savedAt,
            matchScore: scores.get(String(record.job._id))
        }))
    })
}
...
async function analyze(req, res) {
    ...
    if (result.missingProfile) {
        return res.status(422).json({
            ...
            message: 'Create an interview plan with your resume or profile information first.'
        })
    }
...
async function recommended(req, res) {
    ...
    if (!candidate) {
        return res.json({ jobs: [], missingProfile: true })
    }
