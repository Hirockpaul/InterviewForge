...
async function getProgressController(req, res) {
    try {
        const [sessions, timedPractices, focusedPractices, mcqPractices, activityGroups] = await Promise.all([
            mockInterviewModel.find({ user: req.user.id, status: 'completed' })
                .sort({ createdAt: 1 })
                .populate('interviewPlan', 'title')
                .lean(),
            timedPracticeModel.find({ user: req.user.id, status: 'completed' })
                .sort({ createdAt: 1 })
                .lean(),
            focusedPracticeModel.find({ user: req.user.id, status: 'completed' })
                .sort({ createdAt: 1 })
                .populate('questions', 'canonicalTopic difficulty')
                .lean(),
            mcqSessionModel.find({ user: req.user.id, status: 'completed' })
                .sort({ createdAt: 1 })
                .lean(),
            preparationActivityModel.aggregate([
