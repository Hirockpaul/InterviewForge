...
const categoryForMode = {
    technical: ['technical', 'system-design', 'role-specific'],
    behavioral: ['behavioral'],
    project: ['project'],
    mixed: ['technical', 'behavioral', 'project', 'system-design', 'role-specific']
}
...
function publicSession(session) {
    ...
    const elapsed = session.currentQuestionStartedAt
        ? Math.max(0, Math.floor((Date.now() - new Date(session.currentQuestionStartedAt).getTime()) / 1000))
        : 0
    const currentQuestion = question && session.status === 'in-progress'
        ? {
            _id: question._id,
            question: question.question,
            canonicalTopic: question.canonicalTopic,
...
async function sourceFromRequest(req, data) {
    if (data.sourceType === 'resume') {
        if (!req.file) {
            throw httpError('Upload a resume PDF.', 400)
        }
        if (req.file.mimetype !== 'application/pdf') {
            throw httpError('Resume must be a PDF file.', 400)
        }
        try {
            ...
            throw httpError('The resume PDF could not be read. Try a text-based PDF.', 422)
        }
    }
    if (data.sourceType === 'interview-plan') {
        if (!mongoose.isValidObjectId(data.interviewPlanId)) {
            throw Object.assign(new Error('Select a valid interview plan.'), { status: 400 })
        }
        const plan = await interviewReportModel.findOne({ _id: data.interviewPlanId, user: req.user.id })
        if (!plan) {
            throw httpError('Interview plan not found.', 404)
        }
        return [
            `Target role: ${plan.title}`,
            `Candidate resume: ${plan.resume}`,
            `Candidate description: ${plan.selfDescription}`,
            `Job description: ${plan.jobDescription}`
        ].join('\n')
    }
    if (data.sourceText.length < 2) {
        throw httpError('Provide enough information to prepare a session.', 400)
    }
...
async function createSessionController(req, res) {
    try {
        ...
        const topicInput = data.sourceType === 'topic'
            ? sourceText
            : data.sourceType === 'interview-plan'
                ? sourceText.match(/Target role: ([^\n]+)/)?.[1]
        ...
        const visibilityQuery = {
            $or: [
                { visibility: 'global' },
                { visibility: 'user', owner: req.user.id, sourceKey: key }
            ]
        }
        ...
        const baseQuery = {
            ...
            $and: [
                ...
                {
                    $or: [
                        { canonicalTopic: { $in: topicTerms } },
                        { subtopics: { $in: topicTerms } },
                        { skills: { $in: topicTerms } }
                    ]
                }
            ]
        }
        ...
        if (Object.values(missing).some(Boolean)) {
            let generated
            try {
                generated = await generateMissingQuestions({
                    canonicalTopic: topic.canonicalTopic,
                    displayTopic: topic.displayTopic,
                    mode: data.mode,
                    sourceText: data.sourceType === 'topic' ? '' : sourceText,
                    missing,
                    existingNormalized: new Set(available.map(item => item.normalizedQuestion))
                })
            } catch (error) {
            ...
            const documents = generated.map(item => ({
                ...item,
                canonicalTopic: topic.canonicalTopic,
                aliases: topic.aliases,
                visibility,
                owner: visibility === 'user' ? req.user.id : null,
                sourceKey: visibility === 'user' ? key : '',
                qualityStatus: 'approved'
            }))
            if (documents.length) {
                try {
                    await interviewQuestionModel.insertMany(documents, { ordered: false })
                } catch (error) {
                    if (!error.writeErrors?.every(item => item.code === 11000)) {
                        console.warn('Some focused questions were rejected:', error.message)
                    }
                }
            }
        ...
        if (Object.values(missing).some(Boolean)) {
            const needed = Object.entries(missing)
                .filter(([, count]) => count)
                .map(([level, count]) => `${count} ${level}`)
                .join(', ')
            return res.status(503).json({ message: `Insufficient approved questions. Still needed: ${needed}.` })
        }

        const recentAttempts = await questionAttemptModel.find({ user: req.user.id })
            .sort({ attemptedAt: -1 })
            .limit(240)
            .select('question')
            .lean()
        const attemptedIds = new Set(recentAttempts.map(item => String(item.question)))
        const selected = selectQuestions(available, attemptedIds)
        if (selected.length !== 24) {
            return res.status(503).json({ message: 'Unable to assemble a complete 24-question session.' })
        }
        const now = new Date()
        const session = await focusedSessionModel.create({
            user: req.user.id,
            ...
            displayTopic: topic.displayTopic,
            mode: data.mode,
            questions: selected.map(item => item._id),
            currentQuestionStartedAt: now,
            startedAt: now
        })
        await interviewQuestionModel.updateMany(
...
async function getSessionController(req, res) {
    ...
    const session = await focusedSessionModel
        .findOne({ _id: req.params.id, user: req.user.id })
        .populate('questions')
    if (!session) {
        return res.status(404).json({ message: 'Focused practice session not found.' })
    }
...
async function evaluateCompletedSession(sessionId, userId) {
    ...
    for (let index = 0; index < items.length; index += 8) {
        evaluations.push(...await evaluateBatch(items.slice(index, index + 8)))
    }
    const evaluationMap = new Map(evaluations.map(item => [item.questionId, item]))
    session.answers.forEach(answer => {
        answer.evaluation = evaluationMap.get(String(answer.question))
    })
    ...
    await session.save()
    await questionAttemptModel.insertMany(session.answers.map(answer => ({
        user: userId,
...
async function submitAnswerController(req, res) {
    ...
    if (!parsed.data.answer && !parsed.data.submittedAutomatically) {
        return res.status(400).json({ message: 'Enter an answer before submitting.' })
    }

    const existing = await focusedSessionModel
        .findOne({ _id: req.params.id, user: req.user.id, status: 'in-progress' })
        .select('+sourceText')
    if (!existing) {
        const found = await focusedSessionModel.findOne({ _id: req.params.id, user: req.user.id })
        return res.status(found ? 409 : 404).json({
            message: found
                ? 'This session is no longer accepting answers.'
                : 'Focused practice session not found.'
        })
    }

    const index = existing.currentQuestionIndex
    if (String(existing.questions[index]) !== parsed.data.questionId) {
        return res.status(409).json({ message: 'This question has already been submitted or is not current.' })
    }

    const submittedAt = new Date()
    const startedAt = existing.currentQuestionStartedAt
    const question = await interviewQuestionModel.findById(parsed.data.questionId)
        .select('difficulty')
        .lean()
    const limit = timeLimitFor(question.difficulty)
    ...
    const updated = await focusedSessionModel.findOneAndUpdate(
        { _id: existing._id, user: req.user.id, status: 'in-progress', currentQuestionIndex: index },
        {
            ...
            $set: {
                currentQuestionStartedAt: submittedAt,
                ...(isLastQuestion ? { status: 'evaluating' } : {})
            }
        },
        { new: true }
    )
    ...
    if (isLastQuestion) {
        try {
            await evaluateCompletedSession(updated._id, req.user.id)
        } catch (error) {
            console.error('Focused final evaluation failed:', error.message)
            return res.status(503).json({ message: 'Answers were saved, but final evaluation is temporarily unavailable. Retry by reopening this session.' })
        }
    }
    const session = await focusedSessionModel
...
async function getReportController(req, res) {
    ...
    let session = await focusedSessionModel
        .findOne({ _id: req.params.id, user: req.user.id })
        .populate('questions')
    if (!session) {
        return res.status(404).json({ message: 'Focused practice session not found.' })
    ...
    if (session.status === 'evaluating' && session.answers.length === 24) {
        try {
            session = await evaluateCompletedSession(session._id, req.user.id)
            await session.populate('questions')
        } catch {
            return res.status(503).json({ message: 'Evaluation is temporarily unavailable. Your answers are safe; retry shortly.' })
        }
    }
    if (session.status !== 'completed') {
        return res.status(409).json({ message: 'Complete all 24 questions before viewing the report.' })
    }
...
async function listSessionsController(req, res) {
    const sessions = await focusedSessionModel.find({ user: req.user.id })
        .sort({ createdAt: -1 })
        .select('-sourceText -questions -answers')
        .lean()
