...
function fallbackQuestion(interviewPlan, questionIndex = 0) {
  ...
  const isBehavioral =
    questionIndex >= technicalQuestions.length &&
    behavioralQuestions.length > 0;

  return {
    question:
      selected?.question ||
      `What experience best demonstrates your readiness for the ${interviewPlan.title} role?`,
    questionType: isBehavioral ? "behavioral" : "technical",
    topic:
      interviewPlan.skillGaps?.[
        questionIndex % (interviewPlan.skillGaps.length || 1)
      ]?.skill || interviewPlan.title,
    difficulty:
      questionIndex < 2 ? "easy" : questionIndex < 4 ? "medium" : "hard",
  };
}
...
function fallbackEvaluation({ interviewPlan, questions, answer }) {
  ...
  const includesExample =
    /\b(example|for instance|when i|i built|i implemented|result|outcome)\b/i.test(
      answer,
    );
  const includesReasoning =
    /\b(because|therefore|trade-?off|reason|so that)\b/i.test(answer);
  const completeness = Math.min(100, 35 + wordCount * 1.5);
  const communication = Math.min(
    100,
    45 + sentenceCount * 6 + (includesExample ? 12 : 0),
  );
  const depth = Math.min(
    100,
    35 +
      (includesReasoning ? 25 : 0) +
      (includesExample ? 20 : 0) +
      Math.min(wordCount, 40) / 2,
  );
  const relevance = Math.min(
    100,
    55 + (includesExample ? 20 : 0) + (includesReasoning ? 15 : 0),
  );
  const technicalAccuracy = Math.round((completeness + depth) / 2);
  const clarity = Math.round((communication + relevance) / 2);
  const score = Math.round(
    (technicalAccuracy + communication + clarity + depth + relevance) / 5,
  );
    ...
    evaluation: {
      ...
      star: isBehavioral
        ? {
            ...
            result: /\b(result|outcome|improved|reduced|increased)\b/i.test(
              answer,
            )
              ? 80
              : 45,
      ...
      improvements: [
        wordCount < 40
          ...
          : "Make the main conclusion more concise.",
      ],
      feedback:
        "This evaluation was generated locally because the AI service is unavailable. Strengthen the answer with specific decisions, trade-offs, and measurable results.",
    },
...
async function generateOpeningQuestion(interviewPlan, focus = null) {
  try {
    ...
    console.warn(
      "Gemini opening question unavailable; using plan question:",
      error.message,
    );
...
Evaluate the answer constructively. For behavioral questions, score each STAR element; otherwise return null for STAR. Then ask one concise follow-up question influenced by this answer and vary difficulty based on performance.`,
    ...
    console.warn(
      "Gemini answer evaluation unavailable; using local evaluation:",
      error.message,
    );
