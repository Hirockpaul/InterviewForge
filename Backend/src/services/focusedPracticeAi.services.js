...
const generatedQuestionSchema = z.object({
  ...
  subtopics: z.array(z.string().min(1)).min(1).max(8),
  skills: z.array(z.string().min(1)).min(1).max(8),
  intent: z.string().trim().min(8).max(1000),
  expectedPoints: z.array(z.string().trim().min(1)).min(2).max(12),
});
const generationSchema = z.object({
  questions: z.array(generatedQuestionSchema).max(36),
});
...
const evaluationSchema = z.object({
  ...
  categoryScores: z.object({
    ...
    technicalUnderstanding: z.number().min(0).max(100),
    ownership: z.number().min(0).max(100),
    implementationKnowledge: z.number().min(0).max(100),
    ...
    caching: z.number().min(0).max(100),
...
const evaluationBatchSchema = z.object({
  evaluations: z.array(evaluationSchema).max(8),
});
...
async function generateMissingQuestions({
  ...
  const requested = Object.entries(missing)
    .filter(([, count]) => count > 0)
    .map(([difficulty, count]) => `${difficulty}: ${count}`)
    .join(", ");
  const privateContext = sourceText
    ? `Candidate-private source context:\n${sourceText}`
    : "";
...
${privateContext}`,
    config: {
      ...
      responseJsonSchema: z.toJSONSchema(generationResponseSchema),
    },
  });
  ...
  for (const question of parsed.questions) {
    ...
    seen.add(normalized);
    accepted.push({ ...question, normalizedQuestion: normalized });
  }
  return accepted;
}
...
function localEvaluation(item) {
  ...
  const covered = expected.filter((point) =>
    answer.toLowerCase().includes(point.toLowerCase().split(/\s+/)[0]),
  ).length;
  const relevance = expected.length
    ? Math.round(35 + (65 * covered) / expected.length)
    : Math.min(100, 30 + words);
  const completeness = Math.min(
    100,
    Math.round(20 + words * 1.4 + covered * 8),
  );
  const clarity = words
    ? Math.min(90, 45 + Math.round(Math.min(words, 60) / 3))
    : 0;
  ...
  return {
    ...
    strengths: answer
      ? ["Provided a response within the interview time limit."]
      : [],
    weaknesses: answer
      ? ["Cover more of the expected concepts explicitly."]
      : ["No answer was provided."],
    feedback: answer
      ? "Connect the main conclusion to the expected concepts and support it with a concrete example or trade-off."
...
async function evaluateBatch(items) {
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        responseMimeType: "application/json",
        responseJsonSchema: z.toJSONSchema(evaluationBatchSchema),
      },
    });
    const parsed = evaluationBatchSchema.parse(JSON.parse(response.text));
    const byId = new Map(
      parsed.evaluations.map((item) => [item.questionId, item]),
    );
    return items.map(
      (item) => byId.get(String(item.questionId)) || localEvaluation(item),
    );
  } catch (error) {
    console.warn(
      "Focused practice batch evaluation unavailable; using existing deterministic evaluation approach:",
      error.message,
    );
