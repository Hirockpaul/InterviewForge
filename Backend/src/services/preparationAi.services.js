...
const projectListSchema = z.object({ projects: z.array(z.object({ name: z.string().min(1), context: z.string().min(10) })).max(12) })
const questionSetSchema = z.object({ questions: z.array(z.object({
    question: z.string().min(10),
    difficulty: z.enum([ 'beginner', 'intermediate', 'advanced', 'deep-dive' ]),
    topic: z.enum([ 'Architecture', 'Technology choices', 'Database', 'API', 'Security', 'Scalability', 'Performance', 'Challenges', 'Trade-offs', 'Testing', 'Deployment' ])
})).min(4).max(24) })
...
async function generateIntroduction(plan, { targetRole, duration, tone, candidateName }) {
    return structuredGenerate(introductionSchema, `Create a truthful ${duration}-second, ${tone} "Tell me about yourself" introduction for ${targetRole}.
...
async function detectProjects(plan) {
    ...
    return structuredGenerate(projectListSchema, `Extract only projects explicitly described or named by the candidate. Preserve their real names and summarize only stated facts. Do not invent functionality or technologies. Return no project if none exists.\n\n${planContext(plan)}`)
}

async function generateProjectQuestions(plan, project) {
    return structuredGenerate(questionSetSchema, `Generate interview questions about the candidate's project below, grouped across beginner, intermediate, advanced, and deep-dive difficulty. Use ONLY stated project facts; questions may probe decisions, risks, trade-offs, and hypothetical scaling but must not assert unstated functionality. Prioritize relevance to ${plan.title}.\nProject: ${project.name}\nKnown facts: ${project.context}\n\n${planContext(plan)}`)
}

module.exports = { generateIntroduction, detectProjects, generateProjectQuestions }
